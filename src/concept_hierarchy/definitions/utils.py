    ...
    *,
    ...
    if not allow_empty and ch_name == "":
        if return_false_instead_of_error:
            return False
        raise CHSyntaxError(f"The name of a {name_type} cannot be empty!")
    ...
    if not allow_starting_with_digit and ch_name[0].isnumeric():
        if return_false_instead_of_error:
            return False
        raise CHSyntaxError(f"{capitalize(name_type)} names may not start with a digit!")
    if not allow_starting_with_underscore and ch_name.startswith("_"):
        if return_false_instead_of_error:
            return False
        raise CHSyntaxError(f"{capitalize(name_type)} names may not start with an underscore '_' character!")
    ...
    if must_start_lowercase and not ch_name[0].islower():
        if return_false_instead_of_error:
            return False
        raise CHSyntaxError(f"{capitalize(name_type)} name {ch_name!r} must start with a lowercase letter!")
    if must_start_uppercase and not ch_name[0].isupper():
        if return_false_instead_of_error:
            return False
        raise CHSyntaxError(f"{capitalize(name_type)} name {ch_name!r} must start with an uppercase letter!")
    return True
