...
### 14.4 An `fEval:` / `fComp:` / `fInst:` marker may only qualify a key that names a Function
A single content key may carry one of the markers `fEval:`, `fComp:` or `fInst:`, saying which of the three readings of a Function-keyed object is meant (evaluation, composition, or the Function value itself). The key is split at its **first** colon, so a string-literal template argument containing a colon is left intact, and any other prefix is not a marker at all. The use is rejected if what follows the marker does not name a _Function_ — a non-Function type, or nothing at all.

A marker *requires* its reading rather than permitting it: the value is rejected if the expected type's instantiation schema accepts it without that reading standing at the value's own location, which is what a schema of `true` would otherwise do silently.
